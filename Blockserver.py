...
class BlockServerHandler():
    ...
    def storeBlock(self, hashBlock):
        ...
        print "Sending storeBlockResponse to Client"
        print ""
    ...
    def getBlock(self, hashString):
        ...
        if hashString in self.hashBlocks:
            ...
            print "Hash block found, returning the hashblock"
            return hb
        else:
    ...
    # Check if block exist in block server 
    def hasBlock(self, hashString):
        print "Checking for block in local data structure..."
        retNum = intNum()
        if hashString in self.hashBlocks:
            retNum.num = 0
        else:
            retNum.num = 1
        print "Sending hasBlockResponse to metadata"
        print ""
        return retNum
